...
type StickerColor = StickerColorKey | "primary" | "card";

function getStickerCardBgClass(color: StickerColor): string {
  if (color === "primary") return "bg-primary text-primary-foreground";
  if (color === "card") return "bg-card";
  return getStickerBgClass(color);
}
...
export function StickerCard({
  ...
  return (
    <motion.div
      ...
      whileHover={
        hoverLift
          ? { y: -5, rotate: 0, boxShadow: "0 12px 0 rgba(0,0,0,0.12)" }
          : undefined
      }
      transition={{ type: "spring", stiffness: 300, damping: 20 }}
      className={cn(
        "rounded-3xl border-2 border-border p-6 text-foreground relative",
        "shadow-sticker-elevated",
        getStickerCardBgClass(color),
        tape && "tape-strip",
        className
      )}
    >
  ...
  return (
    <motion.span
      ...
      className={cn(
        ...
        "rounded-none",
        "shadow-sticker-card-hover",
        "transition-[transform,box-shadow] duration-200 ease-out",
        "hover:shadow-sticker-card-hover-strong hover:-translate-y-0.5",
        "active:shadow-sticker-sm-soft active:translate-y-0.5",
        "animate-wiggle-cta",
