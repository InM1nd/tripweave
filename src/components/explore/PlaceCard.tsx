...
export function PlaceCard({ place, isRecommendation = false, onAddToTrip }: PlaceCardProps) {
    ...
    return (
        <div className={`overflow-hidden group flex flex-col transition-transform duration-300 rounded-2xl p-5 md:p-4 border-2 border-border hover:-translate-y-2 relative h-full flex-1 min-h-[280px] cursor-pointer shadow-sticker-card hover:shadow-sticker-elevated ${categoryInfo.colorClass}`}>
            ...
            <div className="flex flex-col flex-1 gap-4 relative z-10 min-h-0">
                ...
                <div className="flex flex-wrap items-center gap-2 pt-4 mt-auto border-t-2 border-black/10 shrink-0">
                    <a href={googleMapsUrl} target="_blank" rel="noreferrer" title="Open in Maps" className={`flex items-center justify-center h-9 w-9 rounded-full font-bold ${getStickerBgClass("green")} border-2 border-border shadow-sticker-sm hover:-translate-y-px hover:shadow-sticker-card transition-all`}>
                        <MapPin className="h-4 w-4" strokeWidth={3} />
                    </a>
                    {place.url && (
                        <a href={place.url} target="_blank" rel="noreferrer" title="Open source" className={`flex items-center justify-center h-9 w-9 rounded-full font-bold ${getStickerBgClass("blue")} border-2 border-border shadow-sticker-sm hover:-translate-y-px hover:shadow-sticker-card transition-all`}>
                            <LinkIcon className="h-4 w-4" strokeWidth={3} />
                        </a>
                    )}
                    <button type="button" onClick={() => onAddToTrip?.(String(place.id))} className="flex items-center justify-center h-9 w-9 rounded-full font-black bg-foreground text-background border-2 border-border shadow-sticker-sm hover:-translate-y-px hover:shadow-sticker-card transition-all ml-auto" title="Add to Trip">
