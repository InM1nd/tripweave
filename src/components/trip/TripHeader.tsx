...
export function TripHeader({ trip }: TripHeaderProps) {
  ...
  return (
    <div className="relative">
      {/* Cover Image */}
      <div className="relative h-48 md:h-64 lg:h-80 w-full overflow-hidden rounded-b-3xl md:rounded-3xl">
        ...
        <div className="absolute top-4 left-4 z-20 md:hidden">
          <Button variant="ghost" size="icon" className="h-10 w-10 rounded-full bg-black/30 backdrop-blur-md text-white hover:bg-black/50" asChild>
            <Link href="/dashboard">
        ...
        <div className="absolute top-4 right-4 z-20 flex items-center gap-2">
          <Button variant="ghost" size="icon" className="h-10 w-10 rounded-full bg-black/30 backdrop-blur-md text-white hover:bg-black/50">
          ...
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
            <div className="space-y-2">
              <div className="flex items-center gap-2 flex-wrap">
                <Badge className="bg-black/40 hover:bg-black/40 text-white backdrop-blur-md border-0">
                  {daysDuration} days
                </Badge>
                {isUpcoming && daysUntilTrip <= 30 && (
                  <Badge className="bg-gradient-to-r from-emerald-500 to-teal-600 border-0 text-white">
            ...
            <div className="flex items-center gap-3">
              <div className="flex -space-x-2">
                {trip.members.slice(0, 4).map((member, i) => (
                  <Avatar
                    key={member.id}
                    className="h-9 w-9 border-2 border-white ring-0"
                    style={{ zIndex: 4 - i }}
                  >
                ...
                {trip.members.length > 4 && (
                  <div className="h-9 w-9 rounded-full bg-black/50 backdrop-blur-md flex items-center justify-center text-xs font-bold text-white border-2 border-white">
                    +{trip.members.length - 4}
                  </div>
                )}
              </div>
